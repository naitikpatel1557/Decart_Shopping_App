...
class _HomeTabState extends State<HomeTab> {
  ...
  // Fallback dummy products when Firestore is empty
  final List<Product> _fallbackProducts = [
    Product(
      id: '1',
      name: 'Stainless Steel Cookware Set (5 Pieces)',
      price: 1499,
      imageUrls: ['https://images.unsplash.com/photo-1584992236310-6edddc08acff?w=400&q=80'], productId: '', category: '',
    ),
    Product(
      id: '2',
      name: 'Mixer Grinder (750W Powertron)',
      price: 2199,
      imageUrls: ['https://images.unsplash.com/photo-1570222094114-d054a817e56b?w=400&q=80'], productId: '', category: '',
    ),
    Product(
      id: '3',
      name: 'Electric Kettle (1.8L Premium Auto-Cutoff)',
      price: 899,
      imageUrls: ['https://images.unsplash.com/photo-1570222094114-d054a817e56b?w=400&q=80'], productId: '', category: '',
    ),
    Product(
      id: '4',
      name: 'Non-Stick Frying Pan (24cm with Lid)',
      price: 799,
      imageUrls: ['https://images.unsplash.com/photo-1583778176476-4a8b02a64c01?w=400&q=80'], productId: '', category: '',
    ),
  ];
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
              ...
              children: [
                Row(
                  children: [
                    ...
                    const Text(
                      'CATEGORIES',
                      style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, letterSpacing: 1.2, fontFamily: 'Times New Roman'),
                    ),
                  ],
                ),
                GestureDetector(
          ...
          Padding(
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final String catName = category['name'] ?? 'Category';
                final String catImageUrl = category['imageUrl'] ?? '';
                    ...
                    Container(
                      ...
                      child: ClipOval(
                        child: catImageUrl.isNotEmpty
                            ? Image.network(catImageUrl, fit: BoxFit.cover, errorBuilder: (context, error, stackTrace) => const Icon(Icons.broken_image, size: 24))
              ...
              child: Row(
                ...
                children: [
                  const Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('FREE DELIVERY', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
                      SizedBox(height: 4),
                      Text('On all orders above ₹499', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                    ],
                  ),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                    decoration: BoxDecoration(color: const Color(0xFFD1FAE5), borderRadius: BorderRadius.circular(4)),
                    child: const Text('NO CODE', style: TextStyle(color: Color(0xFF047857), fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.0)),
                  )
                ],
              ),
            ),
          ...
          // --- DYNAMIC / FALLBACK TRENDING PRODUCTS GRID ---
          StreamBuilder<List<Product>>(
            stream: _databaseService.getProducts(),
            builder: (context, snapshot) {
              List<Product> productsToDisplay = _fallbackProducts;

              // Use live database items if available
              if (snapshot.hasData && snapshot.data!.isNotEmpty) {
                productsToDisplay = snapshot.data!;
              }
  ...
  Widget _buildProductCard(Product product, int index) {
    final bool isFavorite = index % 3 == 0;
            ...
            Positioned(
              ...
              child: Center(
                child: GestureDetector(
                  onTap: () {
                    if (_currentBannerIndex > 0) _pageController.previousPage(duration: const Duration(milliseconds: 300), curve: Curves.easeInOut);
                  },
                  child: CircleAvatar(radius: 12, backgroundColor: Colors.black.withOpacity(0.3), child: const Icon(Icons.keyboard_arrow_left, color: Colors.white, size: 16)),
                ),
              ),
            ),
            Positioned(
              ...
              child: Center(
                child: GestureDetector(
                  onTap: () {
                    if (_currentBannerIndex < _banners.length - 1) _pageController.nextPage(duration: const Duration(milliseconds: 300), curve: Curves.easeInOut);
                  },
                  child: CircleAvatar(radius: 12, backgroundColor: Colors.black.withOpacity(0.3), child: const Icon(Icons.keyboard_arrow_right, color: Colors.white, size: 16)),
                ),
              ),
            ),
            Positioned(
              ...
              child: Row(
                children: List.generate(_banners.length, (index) {
                  return AnimatedContainer(
                    duration: const Duration(milliseconds: 200), margin: const EdgeInsets.only(left: 4),
                    height: 6, width: _currentBannerIndex == index ? 16 : 6,
